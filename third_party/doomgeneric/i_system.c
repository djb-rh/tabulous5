...
#ifdef DG_TABULOUS
    // The console is not a process: there is nothing to exit to. The engine
    // stays whole so the game can be resumed from the launcher.
    DG_OnQuit();
    return;
#endif
...
#if !defined(_WIN32) && !defined(__MACOSX__) && !defined(__DJGPP__) && !defined(DG_TABULOUS)
...
#elif defined(DG_TABULOUS)
    {
        DG_OnError(msgbuf);  // does not return
    }
#else
    {
