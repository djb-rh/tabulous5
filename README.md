...
Two things in here carry their own terms, both compatible and both already
included:

- **Anton** (`assets/fonts/Anton-Regular.ttf` and the generated
  `src/font_anton*.h`) is under the SIL Open Font License 1.1. Its licence is at
  `assets/fonts/Anton-OFL.txt` and it stays under OFL whatever the rest of the
  project uses.
- **The sound effects** in `data/sfx/` are generated by `tools/make_sfx.py` from
  plain arithmetic — no sample is derived from any third-party recording — so
  they are covered by the project licence with no attribution requirement. See
